...
function initApp(defaults, app) {
	...
	if (!(defaults.anomaly === false)) {
		...
		var doMailFlag = !!defaults.anomaly // true if anomaly feature enabled
		if (doMailFlag && defaults.anomaly.noEmail != null) {
			if (typeof defaults.anomaly.noEmail == 'string') {
				var date = new Date(defaults.anomaly.noEmail).getTime()
				if (isNaN(date)) doMailFlag = true
				else {
		...
		anomaly.enableAnomalyMail(doMailFlag)
