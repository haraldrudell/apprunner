...
var apimanager = require('./apimanager-x')
...
;[
initApp, getAppName, getLaunchFolder, getAppID
...
function initApp(defaults, app) {
	...
	appName = defaults.init && defaults.init.appName || '?'
	appId = defaults.init && defaults.init.identifier || 'x'

	// configure error handling
	Error.stackTraceLimit = Infinity
	appshutdown.init(defaults)
	if (app) apperror.addErrorListener(app)
	...
	apimanager.loadOnLoad()
	apionloader.doOnLoads(defaults.init.logger)
}
...
function getLaunchFolder() {
	...
	if (folder) folder = path.dirname(folder)
		...
		folder = ~pos ? __dirname.substring(0, pos) : __dirname
	}
	return folder
}