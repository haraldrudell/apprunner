...
function log() { // prepend appName
	logger.apply(this, [appName].concat(util.format.apply(this, Array.prototype.slice.call(arguments))))
}
...
function processSigUsr2() {
	...
	function writeResult(err) {
		if (!err && infoLog) log('Wrote', keys, 'data points at:', (new Date).toISOString())
