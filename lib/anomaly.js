...
function enableAnomalyMail(flag) {
	...
	// notify if off state
	if (!isEmailOn()) {
		var display = flag === false ? flag : (new Date(flag)).toISOString().substring(0, 10)
...
function sendNow() {
	...
	// send email
	if (isEmailOn()) {
		if (sendMail) sendMail('Anomaly Report', body)
		else console.log('mail shutdown:', body)
	}
